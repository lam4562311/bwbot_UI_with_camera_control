...
class MainWindow(QMainWindow, Ui_MainWindow):
    ...
    def __init__(self, parent=None):
        ...
        try:
            ...
            self.control_thread = threading.Thread(target=self.AGV_control_thread, args=(self.control_event, self.x, self.angle, self.speed, 0.5))
...
# End of Camera Control
    ...
    def AGV_control_thread(self, control_event, x, angle, speed, timeout):
        while True:
            
            control_event.wait(timeout)
            self.agv.PUT_robot_speed(x = x * speed, angle = angle * speed)
            control_event.wait.clear()
            logging.debug('speed_x: {}, speed_angle: {}, speed_factor: {}'.format(self.x, self.angle, self.speed))
